		...
		<div class="navbar">
			...
			<div class="navbar-right-container" style="display: flex;">
				<div class="navbar-menu-container">
					...
					<a href="javascript:void(0)" class="navbar-link" @click="loginOut" v-if="nickName">登出</a>
					...
					<div class="confirm-tips">
						...
						<ul>
							<li class="regi_form_input">
								...
								<input type="text" name="loginname" v-model="userName" value="admin" class="regi_login_input regi_login_input_left" placeholder="User Name">
							</li>
							<li class="regi_form_input noMargin">
								<i class="icon IconPwd"></i>
								<input type="password" name="password" v-model="userPwd"  class="regi_login_input regi_login_input_left login-input-no input_text" @keyup.enter="login" placeholder="Password" >
...
<script>
	...
	export default {
		...
		mounted () { // 每次刷新时校检登录和获取购物车数量
			this.checkLogin()
			 
		},
		computed: {
			...mapState(['nickName', 'cartCount']) // mapState实际是对计算属性的封装,['nickname'] 实际就是返回对应的状态值
			// nickName () {
		...
		methods: {
			checkLogin () { // 每次刷新校检是否登录
				...
				.then((response) => {
					...
					if (res.status == '0') {
						...
						this.getCartCount() // 每次刷新时加载购物车数量而不是累加
					}
				})
			},
			login () { // 登录
			...
			getCartCount () {
				axios.get("api/users/getCartCount")
				.then((response) => {
					let res = response.data;
					console.log(res.result)
					this.$store.commit('initCartCount', res.result) // 获取购物车数量时commit加载，避免出现累加
				})
			}
		}
	}
