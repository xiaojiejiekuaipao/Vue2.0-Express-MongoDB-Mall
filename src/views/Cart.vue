...
<script>
	...
	export default {
		...
		computed: {
			checkAllFlag () { // 已勾选的商品的数量和购物车所以商品数量比较判断是否全选
				return this.checkedCount == this.cartList.length;
			},
			checkedCount () {
				var i = 0;
				this.cartList.forEach((item) => { // 遍历购物车商品，计算处于勾选状态的商品数量
					if (item.checked == '1') {
						i ++
					}
				});
		...
		methods: {
			...
			editCart (flag, item){ // 修改购物车商品数量及选中状态
				if (flag == 'add') {
					item.productNum ++
				} else if (flag == 'minu') {
			...
			delCartConfirm (item) {  // 删除模态框
				this.delItem = item  // 将商品id保存到全局以便使用
				this.modalConfirm = true
			},
			delCart () {  // 删除购物车商品
				axios.post("api/users/delCart", {
					productId: this.delItem.productId
				})
				.then((response) => {
					let res = response.data;
					if (res.status == '0') { // 成功删除后关闭模态框，初始化商品列表
						this.modalConfirm = false;
						this.init()
						// 删除商品的时候要根据当前商品的数量来删除
			...
			toggleCheckAll () { // 确认全选
                ...
                // 不能使用这种写法了，checkAllFlag是实时计算的属性，如果true取反变成false之后，还没来得及执行下面的所有商品取消勾选，
                // 就实时计算了检测到勾选的商品种数=购物车商品列表的商品种数,就又变成全选了。
				var flag = !this.checkAllFlag; //全选取反
				this.cartList.forEach((item) => {
					item.checked = flag ? '1' : '0'
				})
				flag = flag ? '1' : '0'; // 将布尔值转换成字符串
				
				axios.post('api/users/editCheckAll', {checkAll: flag})
				.then((response) => {
					let res = response.data;
					console.log(res)
					if (res.status == '0') {
