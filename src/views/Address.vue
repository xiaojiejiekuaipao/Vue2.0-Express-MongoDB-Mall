<template>
  <div>
    <nav-header @init="init"></nav-header>
    <nav-bread>
  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul>
            <li v-for="(item, index) in addressListFilter" v-bind:class="{'check': checkIndex==index}" @click="checkIndex=index;selectAddrId=item.addressId">
              <dl>
                <dt>{{item.userName}}</dt>
      ...
      <div class="next-btn-wrap">
        <router-link class="btn btn--m btn--red" v-bind:to="{path: 'orderConfirm', query: {'addressId': selectAddrId}}">确认订单</router-link>
      </div>
    </div>
  </div>
</div>
   <modal :mdShow="isMdShow" @close="closeModal">
   	...
   	<div slot= "btnGroup">
   		<a class="btn btn--m btn--red" href="javascript:;" @click="delAddress()">确认</a>
...
<script>
	...
	export default {
		data () {
			return {
				limit: 3, // 默认显示几条地址
				checkIndex: 0,    // 选中后的地址索引
				selectAddrId: '', // 选中收货地址的地址Id
				addressList: [],
				more: '展开',
				isMdShow: false,
			}
		...
		computed: {
			addressListFilter () {
				// array.slice获取数组片段，并不会修改原数组
				return this.addressList.slice(0, this.limit)
			}
		},
		methods: {
			init () {
				axios.get('api/users/addressList')
				.then((response) => {
					let res = response.data
					if (res.status == '0') {
						this.addressList = res.result
						this.selectAddrId = this.addressList[0].addressId // 向默认选中地址添加地址Id
					}
				})
			},
			expand () { // 展开显示地址列表
				if (this.limit == 3) { // 如果当前展开条数为3条,则将当前地址长度赋给limit，计算属性则更新为显示所有地址
					this.limit = this.addressList.length;
					this.more = '收回'
				} else { // 否则,收回,当前显示地址仍为3条
					this.limit = 3;
					this.more = '展开'
				}
			},
